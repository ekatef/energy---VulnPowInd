#
plot_by_W_Sens<-function(a_plot,m_nom_plot,d_k_plot,W_nom_plot,W_plot,t_ind_range){
	m_clcd<-m_real(m_nom_plot,W_nom_plot,W_plot)
	# t_plot<-seq(from=min(t_range),to=max(t_range),along=W_range)
	t_plot<-seq(from=t_ind_range[1],to=t_range[t_ind_range[length(t_ind_range)]],along=W_range)
	...
	#
	lines(t_range,Sens(a_plot,m_clcd,d_k_plot,W_nom_plot,W_plot,t_range),
		type="l",lty=1,lwd=2,col=plot_palette[i_rainbow],panel.first=grid(col="black"))
...
#
main_plot_Sens_Ppr<-function(a_plot,m_nom_plot,d_k_plot,W_nom_plot,W_plot,t_ind_range){
	...
	plot(t_range[1:t_ind_range[length(t_ind_range)]],Sens(a_plot,m_nom_plot,d_k_plot,W_nom=W_range[14],W=W_plot,t_range[1:t_ind_range[length(t_ind_range)]]),type="l",lty=1,lwd=5,
